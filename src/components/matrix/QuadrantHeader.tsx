    ...
    >
      ...
      <div className="flex-1 min-w-0" aria-hidden="true">
        <div className="flex items-baseline gap-1.5">
          <h3 className={`text-task-title font-semibold uppercase tracking-wider ${HEADER_ACCENT[quadrant]}`}>{label}</h3>
          <span className="text-meta font-medium text-slate-300 dark:text-slate-600">· {count}</span>
        </div>
        <p className="text-subtitle text-slate-400/80 dark:text-slate-500/80">{subtitle}</p>
      </div>

      {/* Collapse toggle */}
      <button
        onClick={onToggleCollapse}
        className="text-meta text-slate-500 dark:text-slate-400 opacity-50 hover:opacity-80 transition-all
